...
def build_encoder_3pool_rl(config, amps_scaling=1):
    ...
    t2_ie = t2_ie*0.1 + t2_my*config['resolution_limit'] # use resolution limit to constrain t2_ie lower bound
    t2_fr = t2_fr + t2_ie*config['resolution_limit']
...
if __name__ == "__main__":
    ...
    config_path = 'configs/resolution_limit.yaml' 
    with open(config_path, 'r') as file:
        config = yaml.safe_load(file)

    encoder = build_encoder_3pool_rl(config['model']['encoder'], amps_scaling=1)
    encoder.summary()